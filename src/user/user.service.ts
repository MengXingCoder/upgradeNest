...
export class UserService {
    ...
    constructor(@InjectRepository(User) private userRepo1: Repository<User>,
    ...
    async create(createUserDto: CreateUserDto) {
        ...
        if (roleIds.length > 0) {
            ...
            if (invalid.length) throw new BadRequestException(`角色不存在:  $ {invalid.join(', ')}`);
        ...
        return this.dataSource.transaction(async (manager) => {
            ...
            const savedUser = await userRepo.save({ username, password: hashedPassword });
    ...
    async create2(createUserDto: CreateUserDto) {
        ...
        if (roleIds.length === 0) {
            ...
            if (invalidIds.length > 0) {
                throw new BadRequestException(`角色ID不存在:  $ {invalidIds.join(', ')}`);
            }
        ...
        // ================================
        if (finalRoleIds.includes(1)) {
            const hasDefaultRole = await this.roleRepository.exist({ where: { id: 1 } });
            if (!hasDefaultRole) {
        ...
        return await this.dataSource.transaction(async (manager) => {
            ...
            await userRoleRepo.insert(userRoleEntities);
